# ======================================================================================================================
# Modifications by authors of Reddy et al. (2020), "Learning human objectives by evaluating hypothetical behaviours":
# - Changed reward structure to step-based penalties in grass (-1) and bonus for new tile visits (+10)
# - Added 'off_road' flag tracking and crash/success indicators in info dict
# - Added 'succ_rew_bonus' and 'crash_rew_penalty' to step reward logic
# ======================================================================================================================
...
class FrictionDetector(contactListener):
    ...
    def _contact(self, contact, begin):
        ...
        if not obj or "tiles" not in obj.__dict__:
            return
        if begin: # if contacted a tile
            self.env.off_road = False
            obj.tiles.add(tile)
            if not tile.road_visited: # if not visited that tile before
                tile.road_visited = True
                self.env.reward += 1000.0 / len(self.env.track)
                self.env.tile_visited_count += 1 # number of tiles visited
        else:
            obj.tiles.remove(tile) # remove the previous tile
            if len(obj.tiles) == 0: # if wasn't in contact with any tile before
